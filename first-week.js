...
export function weekTwoSuggestion({ sessions = 0, target = 0, painReports = 0 } = {}) {
  if (painReports > 0) return "You reported some discomfort, so Week 2 eases off the affected movements and keeps intensity conservative.";
  ...
  if (ratio >= 1) return "You completed every planned session, so Week 2 adds a small, gradual progression (a rep or a touch of load).";
  if (ratio < 0.7) return "Life got busy, and that's normal. Week 2 keeps it lighter (fewer or shorter sessions) to rebuild the rhythm.";
