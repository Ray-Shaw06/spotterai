...
// retired or restricted per key: this deployment's key receives 404 from
// gemini-2.5-flash, while gemini-3.1-flash-lite accepts the same multimodal,
// structured-output request. If calls start 404-ing, list what the key supports:
//   curl "https://generativelanguage.googleapis.com/v1beta/models?key=$GEMINI_API_KEY"
const GEMINI_MODEL = "gemini-3.1-flash-lite";
// Keep the fallback on another current model that supports both image input
// and structured JSON output so meal photos can recover from transient limits.
const FALLBACK_MODELS = ["gemini-3.5-flash"];
export const GEMINI_MODELS = Object.freeze([GEMINI_MODEL, ...FALLBACK_MODELS]);
...
// With a deadline, Gemini stops early enough to leave Groq this long to answer.
const GROQ_RESERVE_MS = 8000;
// Not worth starting a Gemini attempt with less time than this left.
 ...
 * @param {number}   [opts.deadlineMs]       - Total budget for the whole call. An
 *   overloaded Gemini can take 15s+ just to return 503, so without this the
 *   retries can outlast the serverless limit before Groq is ever tried.
 * @param {number}   [opts.groqReserveMs]    - Share of the deadline kept for Groq.
 * @returns {Promise<string>} The model's text output.
 * @throws  {Error} with `.status` (429 | 503 | 504 | 4xx) when all attempts fail.
 */
async function callGemini({ apiKey, contents, systemInstruction, generationConfig, timeoutMs = 20000, deadlineMs, groqReserveMs = GROQ_RESERVE_MS }) {
  const startedAt = Date.now();
  const groqApiKey = process.env.GROQ_API_KEY;
  const groqEligible = Boolean(groqApiKey) && !hasImagePart(contents);
  const timeLeft = () => (deadlineMs == null ? Infinity : deadlineMs - (Date.now() - startedAt));
  const geminiTimeLeft = () => timeLeft() - (groqEligible ? groqReserveMs : 0);
  ...
  let lastError;
  roster: for (const model of GEMINI_MODELS) {
    for (let attempt = 0; attempt < MAX_TRIES_PER_MODEL; attempt++) {
      const left = geminiTimeLeft();
      ...
      try {
        return await singleCall(endpointFor(model), apiKey, body, Math.min(timeoutMs, left));
      } catch (err) {
        lastError = err;
        // A timeout already consumed the model's entire latency budget. Move to
        // the next model immediately instead of repeating the same long wait;
        // after the roster is exhausted, text requests can still reach Groq.
  ...
  if (groqEligible && lastError && GROQ_FALLBACK_STATUSES.has(lastError.status)) {
    try {
      return await callGroq({ apiKey: groqApiKey, contents, systemInstruction, generationConfig: body.generationConfig, timeoutMs: Math.min(timeoutMs, timeLeft()) });
    } catch {
