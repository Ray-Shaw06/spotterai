...
export const LIMITS = {
  ...
  // Rest push is the one route a person does not choose to call: it fires
  // twice per set, so a hard session is ~120 requests an hour from ONE lifter.
  // Keyed per device rather than per IP (see identityFor), because a gym's wifi
  // and a carrier's NAT put many lifters behind one address, and an IP-keyed
  // budget would have them lock each other out around set 24.
  restPush: { perMinute: 30, perHour: 400 },
  // Food search fires as a person types (debounced), costs no AI quota, and
  // repeat queries are answered by the CDN before they reach this counter.
  foodSearch: { perMinute: 30, perHour: 300 },
  // Turning reminders on books ~22 at once; after that, a few per log.
  reminders: { perMinute: 40, perHour: 300 },
};

/**
 * Routes that do NOT count toward the shared instance ceiling. That ceiling
 * exists to protect one shared Gemini key from a caller rotating IPs; rest push
 * touches no AI quota and is automatic, so letting a busy gym's rest timers
 * refuse someone's plan generation would be the ceiling working backwards.
 * It keeps its own per-caller limits, and api/rest-push.js caps the day.
 * Food search and reminders are the same case: no AI quota, their own
 * per-caller limits, and for reminders a daily cap in api/reminders.js.
 */
const OFF_INSTANCE_BUDGET = new Set(["restPush", "foodSearch", "reminders"]);
 ...
 */
/**
 * Who this request is counted against. The IP by default. For rest push the
 * push endpoint is a far better key: it is per DEVICE, it is already in the
 * body, and it is the thing being rate limited in the first place. Falls back
 * to the IP when there is no endpoint to read (a cancel, or a malformed body),
 * so nothing is ever unmetered.
 */
export function identityFor(route, req) {
  if (route === "restPush" || route === "reminders") {
    const endpoint = req?.body?.subscription?.endpoint;
    if (typeof endpoint === "string" && endpoint) return `sub:${endpoint}`;
  }
...
export function checkRateLimit(route, req, now = Date.now()) {
  ...
  const sharesInstanceBudget = !OFF_INSTANCE_BUDGET.has(route);

  if (sharesInstanceBudget && min.total >= INSTANCE_PER_MINUTE) {
    return { retryAfter: secondsLeft(MINUTE_MS), scope: "instance" };
  }

  const id = createHash("sha256").update(`${route}:${identityFor(route, req)}`).digest("hex").slice(0, 32);
  ...
  if (sharesInstanceBudget) min.total += 1;
