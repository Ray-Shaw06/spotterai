...
function splitMacros(kcal, kg, proteinPerKg) {
  return splitFromProteinKcal(kcal, proteinPerKg * kg * 4);
}

/**
 * The one place fat and carbs are derived from a calorie total. Both the
 * calculator and the no-height fallback route through here, so a target can
 * never be assembled by two different sets of rules.
 */
function splitFromProteinKcal(kcal, rawProteinKcal) {
  const proteinKcal = Math.min(rawProteinKcal, kcal * PROTEIN_PCT_MAX);
...
/**
 * Fill in fat and carbs for a calorie total that already has a protein figure.
 *
 * Used by the no-height fallback: without height Mifflin-St Jeor cannot run, so
 * onboarding falls back to nutrition-safety's conservative bodyweight-only
 * suggestion, which supplies calories and protein but no carbs or fat. Left
 * unfilled those stayed at the stock defaults and contradicted their own
 * calorie total, which is the exact defect this module exists to remove.
 *
 * @returns {{protein: number, carbs: number, fat: number}|null}
 */
export function completeMacros({ kcal, protein } = {}) {
  const k = num(kcal);
  const p = num(protein);
  if (!k || k <= 0 || p == null || p < 0) return null;
  return splitFromProteinKcal(k, p * 4);
}
