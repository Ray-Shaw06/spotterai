...
function hasRawHttpsAuthorityWithoutEncoding(value) {
...
export function validateNotificationConfig(env = {}) {
  ...
  return {
    ...
    dedupSecret,
    publicKey,
    configurationId: createHash("sha256").update(publicKey).digest("base64url"),
    allowedOrigin,
    dailyCap,
    wafRuleId,
  };
...
export function validatePushSubscription(value) {
  if (!isRecord(value)
    ...
    || !hasRawHttpsAuthorityWithoutEncoding(value.endpoint)
    || !isRecord(value.keys)
